...
DISKS=""
OUTPUT_PATH="/var/tmp/zfsbenchmark"
RESULTS_DIR=""
	...
	for thread in $NUM_JOBS; do
		for bs in $BLOCK_SIZES; do
			...
			do
				...
				fio sequential_sync_writes.fio --output-format=${FIO_FORMAT} --output="${OUTPUT_PATH}/seq_sync_write_$1.bs-$bs.thread-$thread.run-$i.json"
			done
		done
	done
	...
	for thread in $NUM_JOBS; do
		for bs in $BLOCK_SIZES; do
			...
			do
				...
				echo "[!] BS: $BLOCKSIZE"
				echo "[!] Num jobs: $NUMJOBS"
				bw_value=$(jq '.jobs[0].write.bw' "${OUTPUT_PATH}/seq_sync_write_$1.bs-$bs.thread-$thread.run-$i.json")
				iops_value=$(jq '.jobs[0].write.iops' "${OUTPUT_PATH}/seq_sync_write_$1.bs-$bs.thread-$thread.run-$i.json")
				bw+="$bw_value, "
				iops+="$iops_value, "
			done
...
sequential_async_writes() {
	...
	for thread in $NUM_JOBS; do
		for bs in $BLOCK_SIZES; do
			...
			echo "[!] BS: $BLOCKSIZE"
			echo "[!] Num jobs: $NUMJOBS"
			fio sequential_async_writes.fio --output-format=${FIO_FORMAT} --output="${OUTPUT_PATH}/seq_async_write.bs-$bs.thread-$thread.run-$i.json"
		done
	done
}
...
random_sync_writes() {
	...
	for thread in $NUM_JOBS; do
		for bs in $BLOCK_SIZES; do
			...
			echo "[!] BS: $BLOCKSIZE"
			echo "[!] Num jobs: $NUMJOBS"
			fio random_sync_writes.fio --output-format=${FIO_FORMAT} --output="${OUTPUT_PATH}/random_sync_write.bs-$bs.thread-$thread.run-$i.json"
		done
	done
}
...
random_async_write() {
	...
	for thread in $NUM_JOBS; do
		for bs in $BLOCK_SIZES; do
			...
			echo "[!] BS: $BLOCKSIZE"
			echo "[!] Num jobs: $NUMJOBS"
			fio random_async_writes.fio --output-format=${FIO_FORMAT} --output="${OUTPUT_PATH}/random_async_write.bs-$bs.thread-$thread.run-$i.json"
		done
	done
}
	...
	for thread in $NUM_JOBS; do
		for bs in $BLOCK_SIZES; do
			...
			do
				...
				echo "[!] BS: $BLOCKSIZE"
				echo "[!] Num jobs: $NUMJOBS"
				bw_value=$(jq '.jobs[0].write.bw' "${OUTPUT_PATH}/seq_sync_write.bs-$bs.thread-$thread.run-$i.json")
				iops_value=$(jq '.jobs[0].write.iops' "${OUTPUT_PATH}/seq_sync_write.bs-$bs.thread-$thread.run-$i.json")
				bw+="$bw_value, "
				iops+="$iops_value, "
			done
			echo "[!] Throughput (KiB/s): $bw"
	...
	for thread in $NUM_JOBS; do
		for bs in $BLOCK_SIZES; do
			...
			do
				...
				echo "[!] BS: $BLOCKSIZE"
				echo "[!] Num jobs: $NUMJOBS"
				bw_value=$(jq '.jobs[0].write.bw' "${OUTPUT_PATH}/seq_async_write.bs-$bs.thread-$thread.run-$i.json")
				iops_value=$(jq '.jobs[0].write.iops' "${OUTPUT_PATH}/seq_async_write.bs-$bs.thread-$thread.run-$i.json")
				bw+="$bw_value, "
				iops+="$iops_value, "
			done
			echo "[!] Throughput (KiB/s): $bw"
	...
	for thread in $NUM_JOBS; do
		for bs in $BLOCK_SIZES; do
			...
			do
				...
				echo "[!] BS: $BLOCKSIZE"
				echo "[!] Num jobs: $NUMJOBS"
				bw_value=$(jq '.jobs[0].write.bw' "${OUTPUT_PATH}/random_sync_write.bs-$bs.thread-$thread.run-$i.json")
				iops_value=$(jq '.jobs[0].write.iops' "${OUTPUT_PATH}/random_sync_write.bs-$bs.thread-$thread.run-$i.json")
				bw+="$bw_value, "
				iops+="$iops_value, "
			done
			echo "[!] Throughput (KiB/s): $bw"
	...
	for thread in $NUM_JOBS; do
		for bs in $BLOCK_SIZES; do
			...
			do
				...
				echo "[!] BS: $BLOCKSIZE"
				echo "[!] Num jobs: $NUMJOBS"
				bw_value=$(jq '.jobs[0].write.bw' "${OUTPUT_PATH}/random_async_write.bs-$bs.thread-$thread.run-$i.json")
				iops_value=$(jq '.jobs[0].write.iops' "${OUTPUT_PATH}/random_async_write.bs-$bs.thread-$thread.run-$i.json")
				bw+="$bw_value, "
				iops+="$iops_value, "
			done	
			echo "[!] Throughput (KiB/s): $bw"
